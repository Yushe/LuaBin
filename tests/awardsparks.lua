...
function EFFECT:Init(data)
	...
	if ent and ent:IsPlayer() then
		sound.Play("weapons/physcannon/physcannon_charge.wav", ent:GetPos(), 80, 220)
...
function EFFECT:Think()
	...
	for i=1, math.random(3, 4) do
		local particle = self.Emitter:Add("effects/spark", pos + Vector(math.Rand(-1, 1), math.Rand(-1, 1), 0):GetNormalized() * math.Rand(-64, 64))
		...
		if rand == 1 then
			particle:SetColor(Color(20, 80, 255))
		elseif rand == 2 then
			particle:SetColor(Color(40, 255, 40))
		elseif rand == 3 then
			particle:SetColor(Color(255, 40, 40))
		else
